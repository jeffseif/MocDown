...
###
class Steam:
    def __init__(self, pressure, temperature, massFlowRate, heatedDiameter, hydraulicDiameter, rodDiameter, flowArea):
        ###
...
def CriticalPowerRatioMITCISE4(steam, axialPowers, flowLengths, endIndex):
    ...
    for index in range(onsetIndex + 1, endIndex):
        ...
        ###
        criticalQuality[index] = (Dh / De) * a / (1 + b / boilingLength);
...
###
def CriticalPowerRatioHitachiCISE4(steam, axialPowers, flowLengths, endIndex):
    ###
    # Extract steam properties
    ###
    mdot = steam.massFlowRate;
    ...
    # Calculate static variables
    ###
    fp = max(0, 1.5 - P / 1.379e7);
    b = 0.19875 * (Pcrit / P - 1) ** 0.4 * G * D ** 1.4;
    ###
    # Iterate over spatial regions
    ###
    criticalQuality = Zeros(Pz.shape);
    ...
    for index in range(onsetIndex + 1, endIndex):
        ...
        thermalPower += Pz[index];
        ###
        f = 1 + 0.1 * (boilingLength - fp);
        if G <= 406.8:
            a0 = (1 - P / Pcrit) / (406.8 / 1200.) ** 0.4 * f;
            a = (1 - P / Pcrit) / (G / 1200.) ** 0.4 * f;
            a = 1 - (a - a0) * (G / 406.8);
        elif 406.8 < G <= 1085.:
            a = (1 - P / Pcrit) / (G / 1200.) ** 0.4 * f;
        else:
            a = (1 - P / Pcrit) / (G / 1176.) ** 0.5 * f;
        ###
        criticalQuality[index] = (Dh / De) * a / (1 + b / boilingLength);
        criticalPowerRatio[index] = mdot * hfg * criticalQuality[index] / thermalPower;
    ###
...
def RbwrThBoilingCalculation(self, transportOutputFile):
    ...
    for assembly in assemblys:
        ...
        # Cell # -> mass density
        ###
        ##### Fix for multiple coolant cells
...
### ##### Implement this?  We will probably have to strip out the meat of RbwrThBoilingCalculation and point to that
